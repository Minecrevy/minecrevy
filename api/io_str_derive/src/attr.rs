...
use quote::quote;
use syn::{Attribute, Expr, Path, Token};
    ...
    pub fn parse<'a>(attrs: impl IntoIterator<Item=&'a Attribute>) -> syn::Result<Self> {
        ...
        for attr in attrs {
            ...
            match path.as_ref().map(|s| s.as_str()) {
                Some("io_repr") => {
                    repr = Some(attr.parse_args::<McIoEnum>()?)
                }
                Some("tag") => {
                    tag = Some(attr.parse_args::<McIoTag>()?)
                }
                Some("options") => {
...
impl McIoEnum {
    pub fn as_condition(&self) -> TokenStream {
        ...
        match self {
            McIoEnum::VarInt => {
                let options = crate_ident("minecrevy_io_str", "IntOptions");
                quote! {
...
impl Parse for McIoEnum {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        match kind.get_ident().map(|v| v.to_string()).as_ref().map(|v| v.as_str()) {
            Some("varint") => Ok(Self::VarInt),
            Some("i8") => Ok(Self::I8),
            Some("u8") => Ok(Self::U8),
            _ => Err(syn::Error::new(kind.span(), format!("invalid enum representation: {}", quote! { #kind })))
        }
    }
}
