...
pub fn generate_options(ty: &Type, options: Option<&McIoOptions>, trait_ty: TokenStream) -> TokenStream {
    options
        .map(|options| {
            let options = options.0.iter()
                .map(|McIoOption { path, eq, value }| {
...
pub fn iter_fields(fields: &Fields) -> Box<dyn Iterator<Item=(Member, &Field)> + '_> {
    match fields {
        ...
        Fields::Unnamed(fields) => Box::new(fields.unnamed.iter()
            .enumerate()
            .map(|(idx, field)| (Member::Unnamed(Index { index: idx as u32, span: field.ty.span() }), field))),
        Fields::Unit => Box::new(empty()),
    }
}

pub fn crate_ident(crate_: &str, ident: &str) -> TokenStream {
    let found = crate_name(crate_)
